...
class VideoProcesser:
    ...
    def detect(self, img: np.ndarray):
        img = img[
            self.polygon_rect.y_min : self.polygon_rect.y_max,
            self.polygon_rect.x_min : self.polygon_rect.x_max,
        ]
    ...
    def process_frames(self):
        ...
        intervals_predict = []
        for n_frame in tqdm(range(self.num_frames), desc=f"{self.video_path.name}"):
            ...
            for _, bbox in enumerate(boxes_detect):
                ...
                iou_areas = (
                    polygon_shapely.intersection(bbox_shapely).area / bbox_shapely.area
                )
                right_max_point = Point(*bbox_.point_max)
                contains_point = polygon_shapely.contains(right_max_point)
                iou_areas_thresh = (
                    IOU_AREAS_THRESH_LOW if contains_point else IOU_AREAS_THRESH_HIGH
                )
                if iou_areas > iou_areas_thresh:
                    ...
                    if intervals_predict[-1].prev_num + 1 != n_frame:
                        ...
                        intervals_predict[-1].prev_num = (
                            intervals_predict[-1].prev_num + 1
                        )
                    break
        ...
        logger.info(
            f"Total number of frames with vehicles: {num_frames_vehicles}/{self.num_frames}"
        )
